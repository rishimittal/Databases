...
*/
    public static void readConfig(String configFilePath){
        ...
        try {
            ...
            LinkedHashMap<String,Integer> columnNum=null;
                    ...
                    if(flag && !line.toString().equalsIgnoreCase("END")){
                        if(tab) {
                            ...
                            if(!name.equalsIgnoreCase("PRIMARY_KEY")) {
                                ...
                                columnNum.put(name.toLowerCase(),i);
                                i++;
                                columnData.put(name.toLowerCase(),type.toLowerCase());
                            }else{
                                table.setPrimary_key(type);
                            }
                        }
                    }
    ...
    public static void populateDBInfo(){
        ...
        for(String n : tables){
            try {
                ...
                LinkedHashMap<String,Integer> colNum = null;
                ...
                while (it.hasNext()){
                    ...
                    {
                        colNum=t.getColumnNum();
                        break;
                    }
                }
                /*LinkedHashMap<String,Set<String>> uniqueCols=new LinkedHashMap<String, Set<String>>();

                Iterator it1=colNum.entrySet().iterator();
                while (it1.hasNext()){
                    Map.Entry pairs=(Map.Entry)it1.next();
                    Set<String> colVals=new HashSet<String>();
                    uniqueCols.put(pairs.getKey().toString(),colVals);
                }*/

                List<Set<String>> uniqueCols=new ArrayList<Set<String>>();
                ...
                do{
                    ...
                    if(inp!=-1){
                        ...
                        int j=lineArray.length;
                        for(int i=0;i<j;i++){
                            Set<String> temp=uniqueCols.get(i);
                ...
                fread.close();
                HashMap<String,Integer> uniqueAttr=new LinkedHashMap<String, Integer>();

                Iterator it1=colNum.entrySet().iterator();
                while (it1.hasNext()){
                    Map.Entry pairs=(Map.Entry)it1.next();
                    String cName=pairs.getKey().toString();
                    int cNum=Integer.parseInt(pairs.getValue().toString());
                    uniqueAttr.put(cName,uniqueCols.get(cNum).size());

                    FileWriter fileWriter=new FileWriter(PATH_FOR_DATA+"/"+n+"."+cName);
                    fileWriter.write(uniqueCols.get(cNum).toString().replace("[","").replace("]",""));
                    fileWriter.flush();
                    fileWriter.close();
                }
                ...
                while (it.hasNext()){
                    ...
                    {
                        t.setLines(lineNo);
                        t.setUnique_attr_counts(uniqueAttr);
                         break;
                    }
                }
    ...
    public static void insertRecord(String tableName, String record){
        ...
        updateRecordCount(tableName,record);
    }

    private static void updateRecordCount(String tableName, String record) {
        LinkedHashMap<String,Integer> colNum=null;
        HashMap<String,Integer> uniqueAttr=null;
        Table t=null;
        Iterator<Table> it=tableList.iterator();
        while (it.hasNext()){
            t=it.next();
            ...
            {
                colNum=t.getColumnNum();
                uniqueAttr=t.getUnique_attr_counts();
                break;
            }
        }
        ...
        while (it1.hasNext()){
            Map.Entry pairs=(Map.Entry)it1.next();
            String cName=pairs.getKey().toString();
            int cVal=Integer.parseInt(pairs.getValue().toString());
            try {
                BufferedReader bufferedReader=new BufferedReader(new FileReader(PATH_FOR_DATA+"/"+tableName+"."+cName));
                ...
                int flag=0;
                for(int i=0;i<j;i++){
                    if(lineArray[i].trim().equalsIgnoreCase(recordArray[cVal].trim()))
                    {
                        flag=1;
                        break;
                    }
                ...
                if(flag==0){
                    ...
                    FileWriter fileWriter=new FileWriter(PATH_FOR_DATA+"/"+tableName+"."+cName,true);
                    fileWriter.write(", "+recordArray[cVal]);
                    fileWriter.flush();
                    fileWriter.close();
                    int count=uniqueAttr.get(cName);
                    count++;
                    uniqueAttr.put(cName,count);
                }
            } catch (IOException e) {
                e.printStackTrace();
            }
    ...
    public static void test(){
        ...
        while (it.hasNext()){
            ...
            System.out.println(t.getColumnNum());
        }
    ...
    */
    ...
    public static int getLinesInTable(String tble_name) {
        int l = 0;
        for(Table t: tableList)
        {
            if(t.getName().equalsIgnoreCase(tble_name))
            {
                l = t.getLines();
            }
        }
    ...
    /*  Returns the distinct values for the table and the particular attribute
    *   It does not have error handling , hence a valid input is expected.
    * */
    public static int v(String relation_name, String attribute){
        ...
        while (it.hasNext()){
            t=it.next();
            if(t.getName().equalsIgnoreCase(relation_name))
            {
                uniqueAttr=t.getUnique_attr_counts();
                break;
            }
        }
        distinct_values=uniqueAttr.get(attribute);
