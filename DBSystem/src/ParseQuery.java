...
public class ParseQuery {
    ...
    public static void joinQuery(String query){
        TGSqlParser tgSqlParser=new TGSqlParser(EDbVendor.dbvoracle);
        tgSqlParser.sqltext=query;
        tgSqlParser.parse();
        TSelectSqlStatement tSelectSqlStatement=(TSelectSqlStatement)tgSqlParser.sqlstatements.get(0);
        TJoin tJoin=tSelectSqlStatement.joins.getJoin(0);
        String table1=null,table2=null;
        //System.out.println(tJoin.getTable().toString());
        if(tJoin.getJoinItems().size()==1){
            table1=tJoin.getTable().toString();
            table2=tJoin.getJoinItems().getJoinItem(0).getTable().toString();
            String Condition1,Condition2,temp1,temp2;
            temp1=tJoin.getJoinItems().getJoinItem(0).getOnCondition().getLeftOperand().toString();
            temp2=tJoin.getJoinItems().getJoinItem(0).getOnCondition().getRightOperand().toString();
            //System.out.println(temp1+" "+temp2);
            if(temp1.contains(table1)){
                Condition1=temp1.substring(temp1.indexOf('.')+1);
                Condition2=temp2.substring(temp2.indexOf('.')+1);
            }else {
                Condition1=temp2.substring(temp2.indexOf('.')+1);
                Condition2=temp1.substring(temp1.indexOf('.')+1);
            }
            //System.out.println(table1+" "+Condition1+" "+table2+" "+Condition2);


        List<String> tab1_attr,tab2_attr;
        tab1_attr=new ArrayList<String>();
        tab2_attr=new ArrayList<String>();
        for(int i=0; i < tSelectSqlStatement.getResultColumnList().size();i++){
            ...
            if(temp.contains(table1)){
                tab1_attr.add(temp.substring(temp.indexOf('.')+1));
            }
            else {
                tab2_attr.add(temp.substring(temp.indexOf('.')+1));
            }
        ...
        while (it1.hasNext()){
            System.out.print(table1 + "." + it1.next()+",");
        }
        Iterator<String> it2=tab2_attr.iterator();
        while (it2.hasNext()){
            System.out.print(table2 + "." + it2.next());
            if(it2.hasNext())
                System.out.print(",");
        }
        System.out.println();
        InnerJoin.initialize(table1,Condition1, table2, Condition2); //To be modified
        InnerJoin.printJoinResult(tab1_attr,tab2_attr);
        }
    }
    ...
    public static void main(String arr[]){
        ...
        pq.joinQuery("SELECT countries.id, countries.name, airports.city, airports.id FROM countries INNER JOIN airports ON countries.id=airports.id");
        
//        System.out.println(DBSystem.v("countries", "id"));
//        DBSystem.insertRecord("countries", "\"1\",\"CBT\",\"Kolkata\",\"IN\"");
//        System.out.println(DBSystem.v("countries", "id"));
//        DBSystem.insertRecord("countries","\"1\",\"BNM\",\"JKL\",\"IN\"");
//        System.out.println(DBSystem.v("countries", "id"));
//        System.out.println(DBSystem.v("countries", "name"));
//        DBSystem.insertRecord("countries","\"302556\",\"AO\",\"Angola\",\"AF\"");
//        System.out.println(DBSystem.v("countries", "id"));
//        System.out.println(DBSystem.v("countries", "name"));
//        System.out.println(DBSystem.v("countries", "code"));
//        System.out.println(DBSystem.v("countries", "continent"));

        //InnerJoin.initialize("countries","id", "airports", "id"); //To be modified
        //InnerJoin.printJoinResult();
